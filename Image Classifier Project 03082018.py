...
print ('Total train: ' + str(len(trainloader)))
print ('Total valid: ' + str(len(validloader)))
print ('Total test: ' + str(len(testloader)))
    ...
    for image, label in iter(trainloader):
        ...
        if steps % print_every == 0:
            ...
            for ii, (image, label) in enumerate(validloader):
                inp = Variable(image, volatile=True)
                inp = inp.cuda()
                label = Variable(label, volatile=True)
                label = label.cuda()
                out = model.forward(inp)
                test_loss += criterion(out, label).data[0]
...
def load_checkpoint(filepath, gpu):
    # Enables choice of CPU or GPU functioning
...
def predict(image_path, model, topk=5):
    ...
    topk_index = ps[topk_index]

    return topk_index, name

temp = 'flower_data/test/46/image_00958.jpg'
topk_prob, named_topk_class = predict(temp, model, idx_to_class)
